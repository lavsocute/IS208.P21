...
public void mousePressed(MouseEvent e) {
    if (e.getSource() == btnThem && Validation()) {
        try {
            // Lấy ID tự động tăng từ sequence
            int id = KhachHangDAO.getInstance().getAutoIncrement();
            
            // Tạo đối tượng khách hàng mới với ngày tham gia là ngày hiện tại
            KhachHangDTO newKhachHang = new KhachHangDTO(
                id, 
                tenKH.getText(),
                sdtKH.getText(), 
                diachiKH.getText(),
                new java.util.Date() // Ngày tham gia là ngày hiện tại
            );
            
            // Thêm vào BUS và database
            boolean success = jpKH.khachhangBUS.add(newKhachHang);
        ...
        KhachHangDTO updatedKhachHang = new KhachHangDTO(
            ...
            sdtKH.getText(), 
            diachiKH.getText(),
            kh.getNgaythamgia() // Giữ nguyên ngày tham gia
        );
        
        boolean success = jpKH.khachhangBUS.update(updatedKhachHang);
