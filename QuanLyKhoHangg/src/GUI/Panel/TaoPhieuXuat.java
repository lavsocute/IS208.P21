...
public final class TaoPhieuXuat extends JPanel {
    ...
    public JTextArea textAreaMaChiTiet;
    private JLabel labelMaChiTiet;
    private JPanel content_right_bottom_top;
    private JPanel content_right_bottom_bottom;
    private ArrayList<SanPhamDTO> spList = new ArrayList<>();
    private Vector v;
    private CustomComboCheck cbxMaChiTiet;
    ...
    private ButtonCustom chonMaChiTiet;
    ...
    private void initComponent(String type) {
        ...
        PlainDocument giaXuatDoc = (PlainDocument) txtGiaXuat.getTxtForm().getDocument();
        giaXuatDoc.setDocumentFilter(new NumericDocumentFilter());
        ...
        ButtonCustom scanMaChiTiet = new ButtonCustom("Quét mã", "success", 14);
        ...
        JPanel jpanelMaChiTiet = new JPanel(new BorderLayout());
        jpanelMaChiTiet.setPreferredSize(new Dimension(0, 40));
        jpanelMaChiTiet.setBackground(Color.WHITE);
        jpanelMaChiTiet.setBorder(new EmptyBorder(0, 0, 10, 0));
        jpanelMaChiTiet.add(labelMaChiTiet, BorderLayout.WEST);
        chonMaChiTiet = new ButtonCustom("Chọn Mã", "success", 14);

        chonMaChiTiet.addActionListener(new ActionListener() {
    @Override
    public void actionPerformed(ActionEvent e) {
        ...
        ctpb = ChiTietSanPhamDAO.getInstance().selectByMaSanPham(masp);
        
        if (ctpb == null || ctpb.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Sản phẩm này không còn hàng!");
        } else {
            SelectMaCT selectMaChiTiet = new SelectMaCT(owner, "Chọn Mã Chi Tiết", true, TaoPhieuXuat.this, ctpb);
        }
    }
});        JPanel jPanelChonMaChiTiet = new JPanel(new GridLayout(1, 2));
        jPanelChonMaChiTiet.setPreferredSize(new Dimension(200, 0));
        jPanelChonMaChiTiet.setOpaque(false);
        jPanelChonMaChiTiet.add(chonMaChiTiet);
        jPanelChonMaChiTiet.add(scanMaChiTiet);

        jpanelMaChiTiet.add(panelScanCenter, BorderLayout.CENTER);
        jpanelMaChiTiet.add(jPanelChonMaChiTiet, BorderLayout.EAST);

        scanMaChiTiet.addActionListener((ActionEvent e) -> {
            if (spList.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn sản phẩm để quét mã");
            } else {
//                QRCode_Dialog qr = new QRCode_Dialog(owner, "Scan", true, textAreaMaChiTiet);
            }
        });
        textAreaMaChiTiet = new JTextArea();
        textAreaMaChiTiet.getDocument().addDocumentListener(new DocumentListener() {
            @Override
            public void insertUpdate(DocumentEvent e) {
                ...
                for (int i = 0; i < arrMaChiTiet.length; i++) {
                    ...
                    for (ChiTietSanPhamDTO chiTietSanPhamDTO : ctpb) {
                        if (arrMaChiTiet[i].equals(chiTietSanPhamDTO.getMaChiTiet())) {
                            check = true;
                        }
                    }
                    if (!check) {
                        String txt = textAreaMaChiTiet.getText().replaceAll("(" + arrMaChiTiet[i] + ")\n", "");
                        textAreaMaChiTiet.setText(txt);
                    }
                }
            }
        ...
        textAreaMaChiTiet.setBorder(BorderFactory.createLineBorder(new Color(204, 204, 204)));
        this.textAreaMaChiTiet.setEditable(false);
        ...
        content_right_bottom_top.add(jpanelMaChiTiet, BorderLayout.NORTH);
        content_right_bottom_top.add(textAreaMaChiTiet, BorderLayout.CENTER);
    ...
    if (chitietphieu.isEmpty()) {
        ...
        if (input == 0) {
            ...
            // Thêm logic cập nhật số lượng tồn
            for (ChiTietPhieuDTO ct : chitietphieu) {
                SanPhamDTO sp = spBUS.getByMaSP(ct.getMaSanPham());
                sp.setSoluongton(sp.getSoluongton() - ct.getSoLuong());
                spBUS.update(sp); // Cần có phương thức update trong SanPhamBUS
            }
    ...
    // Lấy danh sách chi tiết sản phẩm theo mã sản phẩm
    ArrayList<ChiTietSanPhamDTO> chiTietList = chiTietSanPhamBUS.getAllCTSPbyMasp(sp.getMasp());
    
    // Hiển thị giá xuất dưới dạng số nguyên, không định dạng tiền tệ
    this.txtGiaXuat.setText(Integer.toString((int)sp.getGianhap()));
    this.txtGiaXuat.setEditable(false); // Cho phép chỉnh sửa giá
    
    txtSoluongTon.setText(String.valueOf(sp.getSoluongton()));
    
    // Nếu có chi tiết sản phẩm, lấy mã sản phẩm
    if (!chiTietList.isEmpty()) {
        setMaChiTiet(sp.getMasp());
    }
}
    ...
    public boolean checkInfo() {
        ...
        if (txtMaSp.getText().equals("")) {
            JOptionPane.showMessageDialog(null, "Vui lòng chọn sản phẩm");
            check = false;
        } else if (textAreaMaChiTiet.getText().equals("")) {
            JOptionPane.showMessageDialog(null, "Vui lòng chọn mã chi tiết");
            check = false;
        }
    ...
    // Xử lý giá xuất - lấy giá nguyên từ text field
    int dongia;
    try {
        dongia = Integer.parseInt(txtGiaXuat.getText());
    } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(null, "Giá xuất phải là số nguyên");
        return null;
    }
    
    String[] arrMaChiTiet = textAreaMaChiTiet.getText().split("\n");
    int soLuong = getChiTietSp();
    ChiTietPhieuDTO ctpx = new ChiTietPhieuDTO(maphieu, masp, soLuong, dongia);
    ...
    String text = textAreaMaChiTiet.getText().trim();
    if (text.isEmpty()) {
        return 0;
    }
    
    String[] arrMaChiTiet = text.split("\n");
    int masp = Integer.parseInt(txtMaSp.getText());
    
    for (String maChiTiet : arrMaChiTiet) {
        if (!maChiTiet.trim().isEmpty()) {
            ChiTietSanPhamDTO ct = new ChiTietSanPhamDTO(
                maChiTiet.trim(), 
                masp, 
                0, // Có thể điều chỉnh tùy theo logic nghiệp vụ
                maphieu, 
                0  // Có thể điều chỉnh tùy theo logic nghiệp vụ
            );
    ...
    for (ChiTietPhieuDTO chiTietPhieu : chitietphieu) {
        if (chiTietPhieu.getMaSanPham() == masp) {
            return true;
        }
    }
    return false;
    ...
    public void setMaChiTiet(int masp) {
    ctpb = ChiTietSanPhamDAO.getInstance().selectByMaSanPham(masp);
    SanPhamDTO sp = spBus.getByMaSP(masp);
    txtGiaXuat.setText(sp.getGiaban() + "");
    txtSoluongTon.setText(sp.getSoluongton()+"");
    textAreaMaChiTiet.setText("");
    for (int i = 0; i < ctpb.size(); i++) {
        for (ChiTietSanPhamDTO chiTietSanPhamDTO : chitietsanpham) {
            if (chiTietSanPhamDTO.getMaChiTiet().equals(ctpb.get(i).getMaChiTiet())) {
                textAreaMaChiTiet.append(chiTietSanPhamDTO.getMaChiTiet() + "\n");
            }
        }
    }
}
    ...
    for (int i = 0; i < size; i++) {
        ...
        SanPhamDTO sanPham = spBUS.getByMaSP(ct.getMaSanPham());
    ...
    SanPhamDTO spSel = spBUS.getByMaSP(ctphieu.getMaSanPham());
