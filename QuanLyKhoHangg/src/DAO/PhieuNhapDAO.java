...
public class PhieuNhapDAO implements DAOinterface<PhieuNhapDTO> {
    ...
    public int insert(PhieuNhapDTO t) {
        ...
        try {
            ...
            String sql = "INSERT INTO phieunhap(maphieunhap, thoigian, manhacungcap, nguoitao, tongtien, trangthai) VALUES (?,?,?,?,?,1)";
    ...
    public int cancelPhieuNhap(int maphieu){
        int result = 0;
        ChiTietSanPhamDAO.getInstance().deleteByPhieuNhap(maphieu);
        ...
        for (ChiTietPhieuNhapDTO chiTietPhieuNhapDTO : arrCt) {
            SanPhamDAO.getInstance().updateSoLuongTon(chiTietPhieuNhapDTO.getMaSanPham(), -(chiTietPhieuNhapDTO.getSoLuong()));
        }
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "SELECT phieunhap_seq.NEXTVAL FROM dual";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                result = rs.getInt(1) - 1; // Get the current sequence value
            }
        } catch (SQLException ex) {
            Logger.getLogger(PhieuNhapDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
